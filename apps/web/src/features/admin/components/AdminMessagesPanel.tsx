...
function ChatWindow({ withUserId, withName }: { withUserId: string; withName: string }) {
  ...
  const send = trpc.employee.sendMessage.useMutation({
    onSuccess: () => {
      setBody('')
      void refetch()
      void utils.admin.getAdminConversations.invalidate()
    },
  })
...
export function AdminMessagesPanel() {
  const [selectedConv, setSelectedConv] = useState<{ userId: string; name: string } | null>(null)

  const { data: conversations, isLoading } = trpc.admin.getAdminConversations.useQuery(undefined, {
    ...
    <div className="flex h-[calc(100vh-8rem)] overflow-hidden rounded-xl border border-gray-200 bg-white">
      ...
      >
        <div className="border-b border-gray-100 px-4 py-3">
          ...
          <p className="text-xs text-gray-400">Conversaciones con empleados</p>
        </div>
          ...
          {isLoading ? (
            ...
            conversations.map((c) => (
              <button
                key={c.userId}
                type="button"
                onClick={() => setSelectedConv({ userId: c.userId, name: c.fullName })}
              ...
              >
                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-100">
      ...
      <div className={`flex-1 ${selectedConv ? 'flex' : 'hidden lg:flex'} flex-col`}>
        {selectedConv ? (
          <ChatWindow withUserId={selectedConv.userId} withName={selectedConv.name} />
        ) : (
