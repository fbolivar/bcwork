...
export function ScheduleManager() {
  ...
  return (
    <div className="space-y-6">
      {/* Turnos: a quién, cuándo y desde dónde. Es lo que se crea a diario. */}
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-700">Horarios asignados</h2>
            <p className="text-[11px] text-gray-400">Turnos vigentes y futuros por persona</p>
          </div>
          <button
            type="button"
            onClick={() => setTurno(true)}
      ...
      {/* Plantillas: la configuración reutilizable, con sus alertas. */}
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-700">Plantillas de horario</h2>
            <p className="text-[11px] text-gray-400">
              Horas, días y alertas de desconexión que se reutilizan al crear turnos
            </p>
          </div>
          <button
            type="button"
        ...
        {creating && (
          ...
          >
            <h3 className="text-sm font-semibold text-blue-800">Nueva plantilla</h3>
        ...
        {!isLoading && (schedules ?? []).length === 0 && !creating && (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 py-12">
            ...
            <p className="text-sm text-gray-400">
              Sin plantillas. Creá una o guardá un turno como plantilla.
            </p>
          </div>
        )}
      ...
      {turno && <ShiftModal onClose={() => setTurno(false)} />}
    </div>
  )
}
