...
export const adminRouter = router({
   ...
   *
   * Reescrito el 2026-09-07: tres de los cinco tipos devolvian 400 en
   * produccion. `attendance` pedia work_sessions.duration_seconds (no existe),
   * `productivity` pedia daily_user_metrics.date / neutral_seconds /
   * unproductive_seconds / productivity_score (ninguna existe) y ademas
   * embebia users, que no tiene llave foranea desde esa tabla; `absences`
   * fallaba por el mismo embed. El filtro por departamento tampoco filtraba:
   * `.eq('users.department', x)` sin `!inner` deja pasar la fila con el embed
   * en null. Ahora los usuarios se cargan una vez y el cruce se hace aqui, que
   * ademas es lo que permite agrupar por area.
   *
   * Cada informe devuelve tres cosas: el detalle, el agregado por
   * departamento y ese mismo agregado del periodo inmediatamente anterior de
   * igual duracion, para que un numero suelto se pueda leer como mejor o peor.
   */
  // El calculo vive en server/reports.ts porque el cron de informes
  // programados corre exactamente lo mismo. Ver ahi el detalle.
  runCustomReport: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        return await runReport(ctx.db, ctx.user!.tid, input)
      } catch (e) {
        if (e instanceof ReportInputError) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: e.message })
        }
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: e instanceof Error ? e.message : 'Error generando el informe',
        })
      }
    }),
  ...
  // La tabla scheduled_reports existía desde el esquema inicial pero nadie
  // escribía en ella y nada la leía para enviar: el circuito estaba abierto de
  // punta a punta. El envío lo hace /api/cron/scheduled-reports.
  upsertReportSchedule: adminProcedure
    .input(
      z.object({
        id: z.string().uuid().optional(),
        name: z.string().min(2).max(120),
        report_type: z.enum(REPORT_TYPES),
        frecuencia: z.discriminatedUnion('tipo', [
          z.object({
          ...
          z.object({
            ...
            hora: z.number().int().min(0).max(23),
            minuto: z.number().int().min(0).max(59),
            diaMes: z.number().int().min(1).max(28),
          }),
        ]),
    ...
    .mutation(async ({ ctx, input }) => {
      const tid = ctx.user!.tid
      const cron = toCron(input.frecuencia)
      const tz = await getTenantTimezone(ctx.db, tid)
      const ahora = new Date()
      const proxima = proximaEjecucion(cron, ahora, offsetMinutes(ahora, tz))
      ...
      const fila = {
        ...
        filters: input.department ? { department: input.department } : null,
        format: 'xlsx',
        is_active: input.is_active,
        next_run_at: proxima?.toISOString() ?? null,
        created_by: ctx.user!.sub,
      }
      ...
      if (input.id) {
        const { error } = await ctx.db
          ...
          .eq('tenant_id', tid)
        if (error) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: error.message })
        return { ok: true, id: input.id, next_run_at: fila.next_run_at }
      }
      ...
      const { data, error } = await ctx.db
        ...
        .single()
      if (error) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: error.message })
      return { ok: true, id: data?.id, next_run_at: fila.next_run_at }
    }),

  setReportScheduleActive: adminProcedure
    .input(z.object({ id: z.string().uuid(), is_active: z.boolean() }))
    .mutation(async ({ ctx, input }) => {
      const tid = ctx.user!.tid
      const { data: actual, error: e1 } = await ctx.db
        .from('scheduled_reports')
        .select('cron_expression')
        .eq('id', input.id)
        .eq('tenant_id', tid)
        .single()
      if (e1) throw new TRPCError({ code: 'NOT_FOUND', message: e1.message })

      // Al reactivar hay que recalcular: si estuvo apagada, next_run_at quedó
      // en el pasado y el cron la dispararía de inmediato.
      ...
      if (input.is_active) {
        ...
        next =
          proximaEjecucion(
            actual!.cron_expression,
            ahora,
            offsetMinutes(ahora, tz),
          )?.toISOString() ?? null
      }

      const { error } = await ctx.db
        .from('scheduled_reports')
        .update({ is_active: input.is_active, ...(input.is_active ? { next_run_at: next } : {}) })
        .eq('id', input.id)
        .eq('tenant_id', tid)
      if (error) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: error.message })
      return { ok: true }
  ...
  deleteReportSchedule: adminProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const { error } = await ctx.db
        .from('scheduled_reports')
  ...
  /** Envía la programación ahora mismo, sin tocar su calendario. */
  sendReportScheduleNow: adminProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const { data, error } = await ctx.db
        .from('scheduled_reports')
      ...
      const res = await enviarInformeProgramado(ctx.db, data, new Date())
      if (!res.ok) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: res.motivo })
      return { ok: true, enviados: res.enviados }
    }),
