...
export const adminRouter = router({
  ...
  runCustomReport: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      const tid = ctx.user!.tid
      const { report_type, department, user_ids, compare } = input

      const desde = new Date(input.date_from)
      const hasta = new Date(input.date_to)
      if (Number.isNaN(desde.getTime()) || Number.isNaN(hasta.getTime())) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Rango de fechas invalido' })
      }
      if (hasta < desde) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
      ...
      const { data: usuarios, error: uErr } = await ctx.db
        .from('users')
      ...
      if (report_type === 'attendance') {
        ...
        if (cur.error)
          throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: cur.error.message })
        ...
        const cmp = comparar(
          agrupar(val(cur.data ?? [])),
          prev?.data ? agrupar(val(prev.data)) : null,
        )
        return {
